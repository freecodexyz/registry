...
function buildPoints(shape: PointCloudShape, count: number) {
  ...
  if (shape === 'sphere') {
    ...
    for (let index = 0; index < count; index += 1) set(index, (Math.random() * 2 - 1) * 1.55, 0, (Math.random() * 2 - 1) * 1.55)
  } else {
...
export function PointCloud({ shape = 'mobius', density = 1, color, className }: { shape?: PointCloudShape; density?: number; color?: string; className?: string }) {
